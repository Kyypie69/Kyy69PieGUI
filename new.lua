...
local RebirthTab = Main:CreateTab("REB1RTH - Packs")
local StrengthTab= Main:CreateTab("STR3NGTH - Packs")
local KillerTab = Main:CreateTab("KILL3R")
local FarmTab = Main:CreateTab("F4RM REB & R0CK")
...
FarmTab:AddTextBox("Rebirth Target","Enter number",function(Value)
    ...
    print("Rebirth target set to: " .. targetRebirths)
end)

-- Debug rebirth remote events
local rebirthRemote = game:GetService("ReplicatedStorage"):FindFirstChild("rEvents")
if rebirthRemote then
    local rebirthFunc = rebirthRemote:FindFirstChild("rebirthRemote")
    if rebirthFunc then
        FarmTab:AddLabel("✓ Rebirth remote found")
    else
        FarmTab:AddLabel("✗ Rebirth remote not found")
    end
end

FarmTab:AddToggle("Auto Rebirth", false, function(Value)
    isAutoRebirthing = Value
    print("Auto rebirth toggle: " .. tostring(Value))
    ...
    if Value and targetRebirths > 0 then
        task.spawn(function()
            ...
            while isAutoRebirthing do
                ...
                print("Current rebirths: " .. currentRebirths .. " / Target: " .. targetRebirths)
                ...
                -- Try multiple rebirth methods
                local success = false
                
                -- Method 1: Standard rebirth remote
                pcall(function()
                ...
                task.wait(0.2) -- Longer delay for stability
            end
        end)
...
local petOptions = {
    "Darkstar Hunter", "Neon Guardian", "Blue Pheonix", "Crimson Falcon", "Cybernetic Showdown Dragon", "Dark Golem",
    "Dark Legends Manticore", "Eternal Strike Leviathan", "Frostwave Legends Penguin",
    "Gold Warrior", "Golden Viking", "Infernal Dragon", "Muscle Sensei"
}
...
local auraOptions = {
    "Entropic Blast", "Muscle King", "Astral Electro", "Azure Tundra", "Dark Electro", "Dark Lightning", "Dark Storm", "Electro", "Enchanted Mirage",
    "Eternal Megastrike", "Grand Supernova", "Inferno", "Lightning",
    "Power Lightning", "Purple Nova", "Supernova"
}
...
-- Update players when they join/leave
