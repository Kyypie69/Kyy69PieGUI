-- Fixed library URL (removed trailing space)
...
-- ===== SETTINGS =====
local settings = {
    primary = {active = false, orb = nil, city = nil, speed = 1000, cooldown = 0.5, adaptive = true},
    secondary = {active = false, orb = nil, city = nil, speed = 1000, cooldown = 0.5, adaptive = true},
    third = {active = false, orb = nil, city = nil, speed = 1000, cooldown = 0.5, adaptive = true},
    bullMode = false,
    ...
    antiLag = false,
    maxPingThreshold = 150, -- Max acceptable ping in ms
    farmThrottle = false
}
...
-- ===== ADVANCED PING STABILIZER SYSTEM =====
...
function getAdaptiveCooldown(baseCooldown)
    if not settings.pingStabilizer then return baseCooldown end
    
    local ping = networkStats.currentPing
    local threshold = settings.maxPingThreshold
    
    if ping > threshold * 2 then
        -- Critical ping - heavy throttle
        return math.max(baseCooldown * 3, 2.0)
    elseif ping > threshold * 1.5 then
        -- High ping - moderate throttle
        return math.max(baseCooldown * 2, 1.0)
    elseif ping > threshold then
        -- Elevated ping - light throttle
        return math.max(baseCooldown * 1.5, 0.5)
    end
    
    return baseCooldown
end

function getAdaptiveSpeed(baseSpeed)
    if not settings.pingStabilizer then return baseSpeed end
    
    local ping = networkStats.currentPing
    local threshold = settings.maxPingThreshold
    
    if ping > threshold * 2 then
        return math.floor(baseSpeed * 0.3) -- Reduce to 30%
    elseif ping > threshold * 1.5 then
        return math.floor(baseSpeed * 0.5) -- Reduce to 50%
    elseif ping > threshold then
        return math.floor(baseSpeed * 0.7) -- Reduce to 70%
    end
...
function startPingStabilizer()
    ...
    -- Initialize base cooldowns storage
    networkStats.baseCooldowns = {
        primary = settings.primary.cooldown,
        secondary = settings.secondary.cooldown,
        third = settings.third.cooldown
    }
    ...
    loops.pingStabilizer = RunService.Heartbeat:Connect(function()
        ...
        -- Only adjust every 2 seconds to avoid spam
        if now - networkStats.lastOptimization < 2 then return end
        networkStats.lastOptimization = now
        
        -- Dynamic FPS adjustment based on ping
        if setfpscap then
            if ping > settings.maxPingThreshold * 2 then
                setfpscap(30) -- Lower FPS to reduce network load
                networkStats.isThrottled = true
            elseif ping > settings.maxPingThreshold then
                setfpscap(60)
                networkStats.isThrottled = true
            else
                setfpscap(settings.fpsCap or 120)
                networkStats.isThrottled = false
            end
        end
        
        -- Adjust orb farm speeds dynamically if they're running
        for farmKey, farmLoop in pairs(orbCollectionLoops) do
            if farmLoop and settings[farmKey] and settings[farmKey].active then
                -- Restart with new adaptive settings
                if settings[farmKey].adaptive then
                    stopOrbFarm(farmKey)
                    wait(0.1)
                    startOrbFarm(farmKey)
                end
            end
        end
        
        -- Memory cleanup to reduce lag
        if ping > settings.maxPingThreshold then
    ...
    sendNotification("Ping Stabilizer", "Activated with threshold: " .. settings.maxPingThreshold .. "ms")
end

function stopPingStabilizer()
    if loops.pingStabilizer then
        loops.pingStabilizer:Disconnect()
        loops.pingStabilizer = nil
    end
    ...
    if networkStats.baseCooldowns.primary then
        settings.primary.cooldown = networkStats.baseCooldowns.primary
        settings.secondary.cooldown = networkStats.baseCooldowns.secondary
        settings.third.cooldown = networkStats.baseCooldowns.third
    end
    
    networkStats.isThrottled = false
end

-- ===== CONNECTION ENHANCER SYSTEM =====
...
function startConnectionEnhancer()
    ...
    loops.connection = RunService.Heartbeat:Connect(function()
        ...
        pcall(function()
            ...
            if character then
                -- Set replication focus to character for smoother movement
                LocalPlayer.ReplicationFocus = character.PrimaryPart or character:FindFirstChild("HumanoidRootPart")
                
                -- Network ownership optimization for character parts
            ...
            -- Optimize physics replication rate during high ping
        ...
        -- Packet optimization - batch remote calls during farming
        if settings.connectionEnhancer and networkStats.currentPing > 100 then
            -- Reduce heartbeat frequency during high ping
            wait(0.05)
        end
    end)
    
    -- Additional network optimization loop
    if loops.networkOptimizer then loops.networkOptimizer:Disconnect() end
    loops.networkOptimizer = RunService.Stepped:Connect(function()
        if not settings.connectionEnhancer then return end
        
        pcall(function()
            -- Clear unused network instances
            if #Players:GetPlayers() > 20 and networkStats.currentPing > 120 then
                -- Reduce render distance during high player count + high ping
    ...
    sendNotification("Connection Enhancer", "Network optimization active")
end

function stopConnectionEnhancer()
    if loops.connection then
        loops.connection:Disconnect()
        loops.connection = nil
    end
    
    if loops.networkOptimizer then
        loops.networkOptimizer:Disconnect()
        loops.networkOptimizer = nil
    end
    
    -- Reset replication focus
    pcall(function()
...
function startOrbFarm(farmKey)
    ...
    -- Store base cooldown if not already stored
    if not networkStats.baseCooldowns[farmKey] then
        networkStats.baseCooldowns[farmKey] = config.cooldown
    end
    
    orbCollectionLoops[farmKey] = RunService.Heartbeat:Connect(function()
        if not config.active then return end
        
        -- Get adaptive values based on current ping
        local currentCooldown = getAdaptiveCooldown(networkStats.baseCooldowns[farmKey] or config.cooldown)
        local currentSpeed = getAdaptiveSpeed(config.speed)
        ...
        pcall(function()
            ...
            for i = 1, 300 batchSize do
                ...
                if networkStats.currentPing > settings.maxPingThreshold and i < batchSize then
                    wait(0.02)
                end
            end
        end)
        
        wait(currentCooldown)
    end)
end
...
function startHoopFarm()
    ...
    loops.hoops = RunService.Heartbeat:Connect(function()
        ...
        local success, err = pcall(function()
            ...
            -- Adaptive collection rate based on ping
            local processAll = networkStats.currentPing < settings.maxPingThreshold
            local processed = 0
            local maxHoops = processAll and 100 or 20 -- Limit processing during high ping
            
            for _, hoop in ipairs(hoopFolder:GetChildren()) do
                if hoop.Name == "Hoop" then
                    hideHoopRecursively(hoop, character.HumanoidRootPart.CFrame)
                    processed = processed + 1
                    if processed >= maxHoops then break end
                end
            end
        end)
        ...
        -- Adaptive wait time based on ping
        local waitTime = networkStats.currentPing > settings.maxPingThreshold and 0.2 or 0.1
        wait(waitTime)
    end)
end
...
local function notifyTabAccess(tabName)
    pcall(function()
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Tab Accessed",
            Text = "Opening " .. tabName .. "...",
            Duration = 2
        })
    end)
end
...
Universal_Tools:CreateBox("Max Ping Threshold (ms)", function(value)
    settings.maxPingThreshold = tonumber(value) or 150
    sendNotification("Network", "Ping threshold set to " .. settings.maxPingThreshold .. "ms")
end)
...
Universal_Tools:CreateToggle("Connection Enhancer", function(state)
    ...
    if state then 
        startConnectionEnhancer() 
    else 
        stopConnectionEnhancer() 
    end
end)
...
Auto_Farming:CreateToggle("Enable Auto Racing", function(state)
    ...
    if state then 
        startRaceFarm()
        sendNotification("Auto Race", "Auto racing started!")
    else 
        stopRaceFarm()
        sendNotification("Auto Race", "Auto racing stopped!")
    end
end)
...
Auto_Farming:CreateToggle("Auto Collect Hoops", function(state)
    ...
    if state then 
        startHoopFarm()
        sendNotification("Auto Hoops", "Auto hoop collection started!")
    else 
        stopHoopFarm()
        sendNotification("Auto Hoops", "Auto hoop collection stopped!")
    end
end)
...
for _, farmType in ipairs({"Primary", "Secondary", "Third"}) do
    ...
    Auto_Farming:CreateBox("Cooldown (seconds)", function(value)
        ...
        -- Update base cooldown for adaptive system
        if networkStats.baseCooldowns[key] then
            networkStats.baseCooldowns[key] = settings[key].cooldown
        end
    end)
    
    Auto_Farming:CreateToggle("Adaptive Speed (Ping Based)", function(state)
        settings[key].adaptive = state
        sendNotification("Adaptive Farming", state and "Enabled ping-based throttling" or "Disabled adaptive farming")
    end)
    ...
    Auto_Farming:CreateToggle("Enable " .. farmType .. " Farm", function(state)
        ...
        if state then 
            startOrbFarm(key)
            sendNotification("Orb Farm", farmType .. " orb farm started!")
        else 
            stopOrbFarm(key)
            sendNotification("Orb Farm", farmType .. " orb farm stopped!")
        end
    end)
end
...
Pet_Hatching:CreateToggle("Auto Hatch Pet", function(state)
    ...
    if state then 
        startPetHatching("basic")
        sendNotification("Auto Pets", "Auto pet hatching started!")
    else 
        stopPetHatching("basic")
        sendNotification("Auto Pets", "Auto pet hatching stopped!")
    end
end)
...
Teleportation:CreateDropdown("Select Main Location", mainLocations, function(value)
    ...
    sendNotification("Teleportation", "Teleported to " .. value)
end)
...
Teleportation:CreateDropdown("Server Options", serverOptions, function(value)
    if value == "Lowest Player Count" then 
        teleportToLowestServer()
        sendNotification("Teleportation", "Joining server with lowest player count...")
    elseif value == "Server Hop" then 
        serverHop()
        sendNotification("Teleportation", "Server hopping...")
    elseif value == "Rejoin" then 
        rejoinServer()
        sendNotification("Teleportation", "Rejoining current server...")
    end
end)
...
-- ===== TAB NOTIFICATION CLICK HANDLERS =====
-- Add click notifications to tabs after UI loads
spawn(function()
    wait(1.5)
    
    pcall(function()
        -- Define tab mappings with icon IDs and notification text
        local tabMappings = {
            [96221607452840] = "Miscellaneous Tab",
            [103858677733367] = "Farming Tab",
            [84773625854784] = "Pet Hatching Tab",
            [140134362123695] = "Teleportation Tab",
            [98702116897863] = "Rebirth Tab",
            [139117814373418] = "Settings Tab"
        }
        
        -- Find the main UI ScreenGui
        local mainGui = nil
        for _, gui in ipairs(game.CoreGui:GetChildren()) do
            if gui:IsA("ScreenGui") and (gui.Name:find("JOY") or gui.Name:find("KYY") or gui.Name:find("Library")) then
                mainGui = gui
                break
            end
        end
        ...
        if mainGui then
            -- Look for ImageButtons with our tab icon IDs
            for _, obj in ipairs(mainGui:GetDescendants()) do
                if obj:IsA("ImageButton") then
                    local iconId = tonumber(obj.Image:match("%d+"))
                    if iconId and tabMappings[iconId] then
                        local tabName = tabMappings[iconId]
                        obj.MouseButton1Click:Connect(function()
                            notifyTabAccess(tabName)
                        end)
                    end
                end
            end
...
function startPetHatching(petType)
    ...
    loops.petHatch[petType] = RunService.Heartbeat:Connect(function()
        ...
        -- Adaptive hatching based on ping
        local hatchDelay = networkStats.currentPing > settings.maxPingThreshold and 1.0 or 0.5
...
function startRaceFarm()
    ...
    loops.race = RunService.Heartbeat:Connect(function()
        ...
        -- Adaptive race timing based on ping
        local raceDelay = networkStats.currentPing > settings.maxPingThreshold and 1.0 or 0.5
        ...
        if settings.race.mode == "Teleport" then
            pcall(function()
                ...
                for _, pos in ipairs(positions) do
                    ...
                    wait(networkStats.currentPing > 100 and 0.2 or 0.1)
                end
            end)
        ...
        -- Adaptive wait based on ping
        wait(networkStats.currentPing > 100 and 0.1 or 0.05)
    end)
end
...
function startAutoFillRace()
    spawn(function()
        while settings.race.autoFill do
            -- Adaptive fill rate based on ping
            local fillRate = networkStats.currentPing > settings.maxPingThreshold and 0.05 or 0.01
...
function startGiftCollection()
    ...
    loops.gifts = RunService.Heartbeat:Connect(function()
        ...
        -- Adaptive timing based on ping
        local giftDelay = networkStats.currentPing > settings.maxPingThreshold and 600 or 300
...
function startChestCollection()
    ...
    loops.chests = RunService.Heartbeat:Connect(function()
        ...
        -- Adaptive timing based on ping
        local chestDelay = networkStats.currentPing > settings.maxPingThreshold and 120 or 60
            ...
            if chestsFolder then
                for _, chest in ipairs(chestsFolder:GetChildren()) do
                    ...
                    if clickDetector then
                        ...
                        wait(networkStats.currentPing > 100 and 1.0 or 0.5)
                    end
                end
            end
        end)
...
function startWheelSpin()
    ...
    loops.spinWheel = RunService.Heartbeat:Connect(function()
        ...
        -- Adaptive timing based on ping
        local wheelDelay = networkStats.currentPing > settings.maxPingThreshold and 7200 or 3600
...
function claimAllCodes()
    ...
    -- Adaptive code claiming with delays during high ping
    local codeDelay = networkStats.currentPing > settings.maxPingThreshold and 1.0 or 0.5
...
function startAutoRebirth()
    ...
    loops.rebirth = RunService.Heartbeat:Connect(function()
        ...
        -- Adaptive cooldown based on ping
        local actualCooldown = getAdaptiveCooldown(settings.rebirth.cooldown)
        wait(actualCooldown)
    end)
end
...
function startTargetRebirth()
    ...
    loops.targetRebirth = RunService.Heartbeat:Connect(function()
        ...
        -- Adaptive cooldown based on ping
        local actualCooldown = getAdaptiveCooldown(settings.rebirth.targetCooldown)
        wait(actualCooldown)
    end)
end
...
function performInstantRebirth(amount)
    ...
    -- Adaptive speed based on ping
    local rebirthDelay = networkStats.currentPing > settings.maxPingThreshold and 1.0 or 0.5
